...
void llama_memory_seq_cp_overlay(
        llama_memory_t mem,
          llama_seq_id seq_id_src,
          llama_seq_id seq_id_dst,
             llama_pos p0,
    ...
    // Only llama_kv_cache currently supports a safe overlay copy.
    // Fallback to seq_cp for other memory implementations.
    if (auto * kv = dynamic_cast<llama_kv_cache *>(mem)) {
        kv->seq_cp_overlay(seq_id_src, seq_id_dst, p0, p1);
        return;
    }
