...
class ThemePrefs(sharedPreferences: SharedPreferences) :
    ...
    /**
     * 当 [followSystemDayNightTheme] 被禁用时使用此主题。
     * 这是一个实际上不需要 UI 的内部首选项。
     */
    
    val normalModeTheme = ManagedThemePreference(
        sharedPreferences, "normal_mode_theme_tech_ai", ThemeManager.DefaultTheme
    ).also {
        it.register()
    }
    ...
    val followSystemDayNightTheme = switch(
        ...
        "follow_system_dark_mode_tech_ai",
        false,
