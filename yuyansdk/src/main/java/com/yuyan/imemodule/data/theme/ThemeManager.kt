...
object ThemeManager {
    ...
    val BuiltinThemes = listOf(
        ThemePreset.TechAI_BUSINESS,
        ThemePreset.TechAI_WARM,
        ThemePreset.TechAI_INRERNET,
    ...
    val DefaultTheme = ThemePreset.TechAI_BUSINESS
...
fun setNormalModeTheme(theme: Theme) {
        // 使用 activeTheme 的 setter 而不是直接操作 _activeTheme，以触发 fireChange()
        activeTheme = theme
    ...
    private fun evaluateActiveTheme(): Theme {
        ...
        val useSystem = prefs.followSystemDayNightTheme.getValue()
        LogUtil.d("ThemeManager", "评估主题: 跟随系统=$useSystem, 系统暗黑模式=$isDarkMode")
        
        val theme = if (useSystem) {
            if (isDarkMode) prefs.darkModeTheme else prefs.lightModeTheme
        } else {
            prefs.normalModeTheme
        }.getValue()
        
        LogUtil.d("ThemeManager", "最终决定使用主题: ${theme.name}")
        return theme
    }
