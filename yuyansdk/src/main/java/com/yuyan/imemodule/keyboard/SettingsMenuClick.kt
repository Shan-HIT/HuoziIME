...
fun onSettingsMenuClick(inputView: InputView, skbMenuMode: SkbMenuMode) {
    when (skbMenuMode) {
        ...
        SkbMenuMode.Mnemonic -> {
            val enabled = AppPrefs.getInstance().keyboardSetting.keyboardMnemonic.getValue()
            AppPrefs.getInstance().keyboardSetting.keyboardMnemonic.setValue(!enabled)
            KeyboardManager.instance.switchKeyboard()
        }
        ...
        SkbMenuMode.FloatKeyboard -> {
            EnvironmentSingleton.instance.keyboardModeFloat = !EnvironmentSingleton.instance.keyboardModeFloat
            EnvironmentSingleton.instance.initData()
            KeyboardLoaderUtil.instance.clearKeyboardMap()
            KeyboardManager.instance.clearKeyboard()
            KeyboardManager.instance.switchKeyboard()
        ...
        SkbMenuMode.LLMControl -> {
            inputView.service.toggleControlPanel()
        }
        SkbMenuMode.StyleSwitch -> {
            inputView.service.switchStyle()
        }
        SkbMenuMode.ModeSwitch -> {
            inputView.service.switchGenerationMode()
        }
        SkbMenuMode.MemoryPanel -> {
            inputView.service.openMemoryPanel()
        }
        SkbMenuMode.AutomatedTest -> {
            LogUtil.i("SettingsMenuClick", "Test", "点击自动化测试按钮，开始执行...")
//            inputView.service.runAutomatedTest()
        }
        SkbMenuMode.LoRAOverheadTest -> {
//            inputView.service.runLoRAOverheadTest()
        }
        SkbMenuMode.LoRASwitchLatencyTest -> {
//            inputView.service.runLoRASwitchLatencyTest()
        }
        SkbMenuMode.TopKGenerationCostTest -> {
//            inputView.service.runTopKGenerationCostTest()
        }
        SkbMenuMode.ParallelDecodeEfficiencyTest -> {
//            inputView.service.runParallelDecodeEfficiencyTest()
        }
        else ->{}
    }
}