...
object DecodingInfo {
     ...
     */
    fun reset() {
        LogUtil.logSpecialOperation("DecodingInfo", "RESET", "resetting state, wasAssociate=$isAssociate")
        isAssociate = false
        isReset = true
        activeCandidate = 0
    ...
    fun inputAction(event: KeyEvent) {
        LogUtil.logSpecialOperation("DecodingInfo", "INPUT_ACTION", "keyCode=${event.keyCode}, char='${event.unicodeChar.toChar()}'")
        isReset = false
        activeCandidate = 0
        activeCandidateBar = 0
     ...
     */
    fun deleteAction() {
        LogUtil.logSpecialOperation("DecodingInfo", "DELETE_ACTION", "isEngineFinish=$isEngineFinish, isAssociate=$isAssociate")
        activeCandidate = 0
        activeCandidateBar = 0
    ...
    fun chooseDecodingCandidate(candId: Int): String {
        val candidate = getCandidate(candId)
        LogUtil.logCandidateSelected("DecodingInfo", candId, candidate?.text ?: "", candidate?.comment ?: "")
        activeCandidate = 0
        activeCandidateBar = 0
    ...
    fun updateDecodingCandidate() {
        ...
        // 记录候选词列表更新
        LogUtil.logCandidatesUpdate("DecodingInfo", candidatesLiveData.value ?: emptyList())
    }
